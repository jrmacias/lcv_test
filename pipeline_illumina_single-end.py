...
def trimmomatic(dir, sample, reads, threads="4", phred="phred33", adapters="TruSeq3-SE.fa:2:30:10", leading="30", trailing="30", slidingwindow="5:30", minlen="40"):
	...
	s=" "
	cmd1 = " java -jar /usr/share/java/trimmomatic.jar SE -threads " + threads + s + "-phred33 " + reads + s 
	...
	print(cmd)
	os.system(cmd)
	return(out_trimmed_read)


def q30(dir, sample, reads, out_trimmed_reads, ):
	...
	cmd= "zcat " + reads + " | awk 'NR%4==0' | awk '{for (i=1; i<=length($0); i++) {total++; if (substr($0,i,1) >= \"!\") q30_count++;}} END {printf \"%.2f\\n\", (q30_count/total)*100}'"
	...
	cmd= "zcat " + out_trimmed_reads + " | awk 'NR%4==0' | awk '{for (i=1; i<=length($0); i++) {total++; if (substr($0,i,1) >= \"!\") q30_count++;}} END {printf \"%.2f\\n\", (q30_count/total)*100}'"
	print(cmd)
...
def calculate_q30_percentage_from_compressed_fastq(dir, sample, reads, out_trimmed_reads):
	print("Calculating Q30 bases")
	print("---------------------")
	parent_dir=dir
	out_dir=os.path.join(parent_dir, "primario")
...
def reads_PF_post_trimmeed(dir, sample, reads, out_trimmed_reads):
	...
	count_post_read1 = 0
	with gzip.open(out_trimmed_reads, 'r') as t1:
		for line in t1:
			line=line.rstrip()
			if line.startswith(b'@'):
				count_post_read1+=1
	...
	PF_file.write("Number PF\tNumber no PF\t" + str(count_post_read1)+"\t"+str(PF_read1))
...
def kraken(dir, sample, reads, threads=4):
	print("Start kraken")
	print("------------")
	parent_dir=dir
	taxonomy_dir=os.path.join(parent_dir, "taxonomy")
	...
	#kraken standard db
	cmd="/home/admingenomica/software/kraken2/kraken2 --db /labgenomica/kraken_standard_db_v12_01_2024/ --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_kraken_standard_db.txt --gzip-compressed " + reads + " --report " + taxonomy_dir + "/" + sample + "_kraken_standard_db_report.txt"
	print(cmd)
	os.system(cmd)
	
	#kraken viral db
	cmd2="/home/admingenomica/software/kraken2/kraken2 --db /labgenomica/kraken_viral_db_v12_01_2024/ --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_kraken_viral_db.txt --gzip-compressed " + reads + " --report " + taxonomy_dir + "/" + sample + "_kraken_viral_db_report.txt"
	print(cmd2)
	os.system(cmd2)

	#minikraken
	cmd="/home/admingenomica/software/kraken/kraken --db /labgenomica/minikraken_db/minikraken_20171019_8GB --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_minikraken.txt --fastq-input --gzip-compressed " + reads
	print(cmd)
	os.system(cmd)
	#para virus: 
...
def mash_screen (dir, db, reads):
	print("Start Mash screen")
	print("------------")
	parent_dir=dir
	taxonomy_dir=os.path.join(parent_dir, "taxonomy")

	cmd="mash screen " + db + " " + reads+ " > " + taxonomy_dir +"/mash_out.tab"
	print(cmd)
	os.system(cmd)
...
def spades(dir, sample, reads, threads=4):
	print("Start SPADES")
	print("------------")
	parent_dir=dir
	...
	cmd1="/home/admingenomica/software/SPAdes-3.15.4-Linux/bin/spades.py -t " + threads + " -s " + reads
...
def quast(dir, sample, scaffold, reads, reference="nan", threads=4):  
	print("Start QUAST")
	print("-----------")
	parent_dir=dir
	assembling_dir=os.path.join(parent_dir, "assembling")
	...
	cmd1="bwa mem -R \"@RG\\tID:"+ sample +"\\tSM:"+ sample +"\\tPL:ILLUMINA\" -t " + threads + s + scaffold + s + reads
	...
	cmd="pilon -Xmx200G --genome " + scaffold + " --unpaired " + assembling_dir + "/" + sample + "_assembly_bwa_sort.bam --output " + sample + " --outdir " + assembling_dir
	print(cmd)
	os.system(cmd)
	...
	if not reference == "nan":
		cmd="quast.py " + " -o " + assembling_dir + " -t " +threads + " -r " + reference + " --single " + reads + " " + assembling_dir + "/" + sample + ".fasta"
	else:
		cmd="quast.py " + " -o " + assembling_dir + " -t " +threads +" --single " + reads + " " + assembling_dir + "/" + sample + ".fasta"
	print(cmd)
	os.system(cmd)
...
def patho_typing(dir, reads, threads=4):
	...
	cmd="patho_typing.py -f " + reads + " -j " + threads + " -o " + pathotyping_dir + " -s Escherichia coli"
	print(cmd)
	os.system(cmd)
...
def virulencefinder(dir, assembling, specie="nan"):
	...
	if specie=="nan" or specie != "listeria" or specie != "escherichia_coli":
		cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + assembling + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/virulencefinder_db/ -x"
		print(cmd)
		os.system(cmd)
	else:
		if specie == "listeria":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + assembling + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/virulencefinder_db/ -d listeria -x"
		if specie == "escherichia_coli":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + assembling + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/virulencefinder_db/ -d virulence_ecoli -x"
...
def plasmidfinder(dir, scaffold):
	...
	cmd="/home/admingenomica/software/plasmidfinder/plasmidfinder.py -i " + scaffold + " -o " + plasmidfinder_dir + " -x -p /home/admingenomica/software/plasmidfinder_db/plasmidfinder_db/" + " -t 0.80"
	print(cmd)
	os.system(cmd)
...
def bwa(dir, sample, read, reference, threads=4):
	print("Start bwa")
	print("---------")
	...
	cmd1="bwa mem -R \"@RG\\tID:"+ sample +"\\tSM:"+ sample +"\\tPL:ILLUMINA\" -t " + threads + s + reference + s + read
...
''''''
'''
def coverage_assembly(dir, sample, coveragedir, out_trimmed_reads):
	...
	cmd="zcat " + out_trimmed_reads + " | awk 'NR%4 == 2 {sum+=length($0); n+=1} END { print sum/n}'"
...
def main():
	...
	help_text="Pipeline for NGS analysis of illumina short single reads from microrganism samples. This includes the following steps: raw reads quality analysis and trimming, assembling, genome annotation, mapping with reference genome, variant calling, identification of arm genes..."
	...
	required.add_argument("--reads", "-r", help="fastq file with reads (single-end)", required=True)
	...
	required.add_argument("--specie", "-sp", help="salmonella, escherichia_coli, campylobacter or klebsiella, listeria")
	...
	#run fastqc
	fastqc(output, sample, threads, reads)
	
	#run trimmomatic
	trimmomatic(output, sample, reads, threads)
	
	#run fastqc
	fastqc(output, sample, threads, output+"primario/"+sample+"_trimmed.fastq.gz")

	
	#run calculate q30 results
	#q30(output, sample, reads, output+"/primario/"+sample+"_trimmed.fastq.gz") #con bash - awk
	calculate_q30_percentage_from_compressed_fastq(output, sample, reads, output+"/primario/"+sample+"_trimmed.fastq.gz")
	
	#run calculate number PF reads post trimmed
	reads_PF_post_trimmeed(output, sample, reads, output+"/primario/"+sample+"_trimmed.fastq.gz")
	
	#run kraken
	kraken(output, sample, output+"/primario/"+sample+"_trimmed.fastq.gz", threads)
	
	#no funciona bien MIRAAAAARLO
	confindr(output, "/home/admingenomica/software/confindr_db/")
	
	
	mash_screen (output, "/home/admingenomica/software/mash_screen_db/RefSeq88n.msh", output+"/primario/"+sample+"_trimmed.fastq.gz")
	...
	spades(output, sample, output+"/primario/"+sample+"_trimmed.fastq.gz", threads)
	
	#run quast
	#con referencia
	
	if args.reference:
		quast(output, sample, output+"/assembling/scaffolds.fasta",  output +"/primario/"+sample+"_trimmed.fastq.gz", reference, threads)
		...
		#sin referencia
		quast(output, sample, output+"/assembling/scaffolds.fasta", output +"/primario/"+sample+"_trimmed.fastq.gz", threads=threads)
	...
	coverage_assembly(output, sample, mergeresdir, output+"/primario/"+sample+"_trimmed.fastq.gz")
	...
	if (args.specie) and (specie == "escherichia_coli"):
		...
		patho_typing(output, output +"/primario/"+sample+"_trimmed.fastq.gz", threads)
	...
	if args.reference:
		...
		bwa(output, sample, output+"/primario/"+sample+"_trimmed.fastq.gz", reference, threads)
