...
def kraken(dir, sample, paired_reads1, paired_reads2, threads=4):
	...
	#para virus:
	#cmd="/home/admingenomica/software/kraken/kraken --db /labgenomica/kraken_viruses_db --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_kraken_virus.txt --fastq-input --gzip-compressed --paired " + paired_reads1 + " " + paired_reads2
	#para bacterias
	cmd="/home/admingenomica/software/kraken/kraken --db /labgenomica/minikraken_db/minikraken_20171019_8GB --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_kraken.txt --fastq-input --gzip-compressed --paired " + paired_reads1 + " " + paired_reads2
	print(cmd)
	os.system(cmd)
	#para virus: 
	...
	#para bacterias:
	cmd2="/home/admingenomica/software/kraken/kraken-report --db /labgenomica/minikraken_db/minikraken_20171019_8GB " + taxonomy_dir + "/" + sample + "_kraken.txt > " + taxonomy_dir + "/" + sample+"_kraken_report.txt"
	print(cmd2)
	os.system(cmd2)
...
def pointfinder(dir, scaffold, specie):
	...
	#print(scaffold,"\n",specie,"\n")
	pointfinder_dir=os.path.join(parent_dir, "pointfinder")
	cmd="python3 /home/admingenomica/software/pointfinder/PointFinder.py -i " + scaffold + " -o " + pointfinder_dir + " -s " + specie + " -p /home/admingenomica/software/pointfinder_db -m blastn -t 0.90 -l 0.60 -m_p /home/admingenomica/software/ncbi-blast-2.9.0+/bin/blastn"
...
def resistencias(dir, sample, resistancedir, specie="nan"):
	...
	##############################################
	#Añadir los datos fenotipicos al fichero específico de la especie
	...
	if (specie != "nan"):
		...
		#print(pheno_muestra_list)

		'''
		#print(pheno_dict)
		dict_prueba={"Clase": ["aminoglycoside"],
					"Phenotipo":["Amikasin, Tobramycin"],
					"Genes":["aac(6)"],
					"Sample1":["1"]
					}
		file=pd.DataFrame(dict_prueba)
		print(file)
		'''
		''' REPARAR ESTOO
		if os.path.isfile(resistancedir + "/pheno_resistances_results_" + specie + ".xlsx"):	
			df=pd.read_excel(resistancedir + "/pheno_resistances_results_" + specie + ".xlsx")
			#print("eL DF ES\n",df)
			data=df.to_dict(orient="list")
			print("El data ahora es \n", data) #AAAAAAAAAAAAAAQQQQQQQQQQQQQQQQQQQQQQUUUUUUUUUUUUIIIIIIIIIIIIIIIIIII
			data[sample]=[]
			already_added_genes=[]
			...
			print("En noooowwww el data ahora es \n", data)
			file=pd.DataFrame(data)
			file.to_excel(resistancedir+ "/pheno_resistances_results_" + specie + ".xlsx", index=False)
...
def merge_pointfinder_results(dir, sample, pointfinderdir):
	...
	sample="_"+sample
	pointfinder_file=parent_dir + "/pointfinder/"+sample_ed+"_blastn_results.tsv"
...
def main():
	...
	#run pointfinder ES OBLIGATORIO PASARLE COMO PARAMETRO LA ESPECIE
	if (args.specie) and (specie != "nan"):
		pointfinder(output, output+"/assembling/"+sample+".fasta", specie)
		#pointfinder(output, output+"/hybrid_assembling/assembly.fasta", specie)
