...
def trimmomatic(dir, sample, reads1, reads2, threads="4", phred="phred33", adapters="TruSeq3-PE.fa:2:30:10:2:keepBothReads", leading="30", trailing="30", slidingwindow="5:30", minlen="40"):
...
def spades(dir, sample, paired_reads1, paired_reads2, threads=4):# quite  unpaired_reads1, unpaired_reads2,
	print("Start SPADES")
	print("------------")
	parent_dir=dir
	...
	#estaaaacmd1="/home/admingenomica/software/SPAdes-3.15.4-Linux/bin/spades.py -t " + threads + " -1 " + paired_reads1 + " -2 " + paired_reads2 + " -s " + unpaired_reads1 + " -s " + unpaired_reads2
...
def quast(dir, sample, scaffold, paired_reads1, paired_reads2, reference="nan", threads=4):# quite , unpaired_reads1, unpaired_reads2
	print("Start QUAST")
	print("-----------")
	parent_dir=dir
	assembling_dir=os.path.join(parent_dir, "assembling")
	...
	if not reference == "nan":
		#estaaacmd="quast.py " + " -o " + assembling_dir + " -t " +threads + " -r " + reference + " --pe1 " + paired_reads1 + " --pe2 " + paired_reads2 + " --single " + unpaired_reads1 + " --single " + unpaired_reads2 + " " + assembling_dir + "/" + sample + ".fasta"
		...
		#estaaaacmd="quast.py " + " -o " + assembling_dir + " -t " +threads + " --pe1 " + paired_reads1 + " --pe2 " + paired_reads2 + " --single " + unpaired_reads1 + " --single " + unpaired_reads2 + " " + assembling_dir + "/" + sample + ".fasta"
...
def mlst_DTU(dir, assembling, specie):
	...
	if specie=="escherichia_coli":
		cmd="python3 /home/admingenomica/software/mlst/mlst.py -i " + assembling + " -o " + mlst_dir + " -t " + mlst_dir + " -s ecoli_2 -p /home/admingenomica/software/mlst_db/ -x"
	if specie=="listeria":
...
def resfinder(dir, scaffold, specie="nan"):
	...
	if (specie=="nan"):
		...
		cmd = "python /home/admingenomica/.local/lib/python3.9/site-packages/resfinder/run_resfinder.py -ifa " + scaffold + " -o " + resfinder_dir + " -acq -db_res /home/admingenomica/software/resfinder_db/resfinder_db/ -d -db_disinf /home/admingenomica/software/resfinder_db/disinfinder_db/ -c -db_point /home/admingenomica/software/resfinder_db/pointfinder_db/"
	else:
		if specie == "escherichia_coli":
			specie = "ecoli"
		#version antigua
...
def virulencefinder(dir, assembling, specie="nan"):
	...
	if specie=="nan" or specie != "listeria" or specie != "escherichia_coli":
		cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + assembling + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/ -x"
		print(cmd)
		os.system(cmd)
	else:
		if specie == "listeria":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + assembling + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/ -d listeria -x"
		if specie == "escherichia_coli":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + assembling + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/ -d virulence_ecoli -x"
...
def recycler(dir, sample, assembly_graph):
	...
	cmd="/home/admingenomica/software/plasmidfinder/plasmidfinder.py -i " + recycler_dir + "/assembly_graph.cycs.fasta -o " + plasmidfinder_recycler_dir + " -x -p /home/admingenomica/software/plasmidfinder_db/" + " -t 0.80"
	print(cmd)
	os.system(cmd)
...
def plasmidfinder(dir, scaffold):
	...
	cmd="/home/admingenomica/software/plasmidfinder/plasmidfinder.py -i " + scaffold + " -o " + plasmidfinder_dir + " -x -p /home/admingenomica/software/plasmidfinder_db/" + " -t 0.80"
	print(cmd)
	os.system(cmd)
...
def main():
	...
	#run fastqc
	#estaaaaafastqc(output, sample, threads, trimmed_paired_reads1, trimmed_paired_reads2)
	...
	#mash_screen (output, "/home/admingenomica/software/mash_screen_db/refseq.genomes.k21s1000.msh", reads1, reads2)
	...
	#estaaaaspades(output, sample, output+"/primario/"+sample+"_trimmed_1P.fastq.gz", output+"/primario/"+sample+"_trimmed_2P.fastq.gz", output+"/primario/"+sample+"_trimmed_1U.fastq.gz", output+"/primario/"+sample+"_trimmed_2U.fastq.gz", threads)
	...
	if args.reference:
		#estaaaa quast(output, sample, output+"/assembling/scaffolds.fasta",  output +"/primario/"+sample+"_trimmed_1P.fastq.gz",  output +"/primario/"+sample+"_trimmed_2P.fastq.gz", output +"/primario/"+sample+"_trimmed_1U.fastq.gz", output+"/primario/"+sample+"_trimmed_2U.fastq.gz", reference, threads)
		...
		#sin referencia
		#estaaaaa quast(output, sample, output+"/assembling/scaffolds.fasta", output +"/primario/"+sample+"_trimmed_1P.fastq.gz",  output +"/primario/"+sample+"_trimmed_2P.fastq.gz", output+"/primario/"+sample+"_trimmed_1U.fastq.gz", output+"/primario/"+sample+"_trimmed_2U.fastq.gz", threads=threads)
	...
	#prokka(output, sample, output+"/"+sample+".fasta", threads)
