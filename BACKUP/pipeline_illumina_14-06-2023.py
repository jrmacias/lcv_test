...
def kraken(dir, sample, paired_reads1, paired_reads2, threads=4):
	...
	#para virus:
	#cmd="kraken --db /labgenomica/kraken_viruses_db --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_kraken_virus.txt --fastq-input --gzip-compressed --paired " + paired_reads1 + " " + paired_reads2
	#para bacterias
	cmd="kraken --db /labgenomica/minikraken_db/minikraken_20171019_8GB --threads " + threads + " --output " + taxonomy_dir + "/" + sample + "_kraken.txt --fastq-input --gzip-compressed --paired " + paired_reads1 + " " + paired_reads2
	print(cmd)
	os.system(cmd)
	#para virus: 
...
def sistr(dir,sample,sistrdir, scaffold):
	print("Start sistr")
	print("----------")
...
def resistencias(dir, sample, resistancedir, specie="nan"):
	...
	if (specie != "nan"):
		pheno_muestra_list=[]
		resfinder_pheno_file=parent_dir + "/resfinder/pheno_table_" + specie + ".txt"
...
def main():
	...
	#run plasmidfinder
	#estaaaplasmidfinder(output, output+"/assembling/"+sample+".fasta")
	...
	if args.reference:
		...
		#run variantcalling
		bcftools_varcalling(output, sample, output + "/secundario_bowtie2/" + sample + "_nodup_sort_mapped_rg.bam", reference, threads)
		...
		if args.phages_bed:
			bedtools_filterPhageRegion(output, sample, output + "/terciario_bowtie2/" + sample + "_filter.vcf", phages_bed)
		...
		if args.phages_bed:
			...
			generate_consensus_sequence(output, sample, output + "/terciario_bowtie2/" + sample + "_filter.vcf", reference)
