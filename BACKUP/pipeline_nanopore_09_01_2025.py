...
def longqc(output_nanopore, minion_read, sample, threads):
	...
	print(cmd1)
	os.system(cmd1)
	cmd2= "python /home/admingenomica/software/LongQC/longQC.py sampleqc -o " + out_dir + "/longqc" + " -x ont-ligation -s " + sample + " -p " + threads + s + minion_read
	print(cmd2)
	os.system(cmd2)
...
def fastqc(output_nanopore, minion_read, sample, threads):
	...
	print(cmd1)
	os.system(cmd1)
	cmd2="fastqc -t " + threads + " -o " + fastqc_dir + "/fastqc/" + s + minion_read
	print(cmd2)
	os.system(cmd2)
...
def fasqc_trimming(output_nanopore, filtered_trimming, sample, threads):
	...
	fastqc_dir=os.path.join(parent_dir, "primario")
	s=" "
	cmd1="fastqc -t " + threads + " -o " + fastqc_dir + "/fastqc/" + s + fastqc_dir + "/" + sample + "_trimmed_filtered.fastq.gz"
...
def longqc_trimming(output_nanopore, filtered_trimming, sample, threads):
	...
	s=" "
	cmd="python /home/admingenomica/software/LongQC/longQC.py sampleqc -o " + longqc_dir + "/longqc/longQC_trimmed_reads" + s + "-x ont-ligation -s " + sample + s + "-p " + threads + s + filtered_trimming
...
def assembl_hibrid(output_illumina, output_nanopore, unpair1, unpair2, sample, pair1, pair2, filtered_trimming, threads):
	...
	cmd2="unicycler -1 " + pair1 + s + "-2 " + pair2 + s + "-s " + output_illumina+"/primario/"+sample+"_trimmed_allU.fastq.gz" + s + "-l " + output_nanopore+"/primario/"+ sample+"_trimmed_filtered.fastq.gz" + s + "-o" + s + assembling_dir + s + "-t " + threads
	#sin unpairs
...
def quast(output_illumina, output_nanopore, sample, pair1, pair2, unpair_concatened, filtered_trimming, threads ):
	...
	cmd="quast.py -o " + assembling_dir + s + "-t "+ threads + " --pe1 " + pair1 + s + "--pe2 " + pair2 + s + "--single " + output_illumina+"/primario/"+sample+"_trimmed_allU.fastq.gz --nanopore " + parent_dir_Nanopore + "/primario/" + sample + "_trimmed_filtered.fastq.gz" + s + assembling_dir + "/assembly.fasta"
	print(cmd)
	os.system(cmd)
