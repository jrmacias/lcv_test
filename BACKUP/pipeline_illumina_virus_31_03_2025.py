...
def segment_coverage(dir,sample, mergeresdir, referencia):
	secundario_dir=os.path.join(dir, "secundario")
	
	ruta_abs=os.path.abspath(referencia)
	ref_dir=os.path.dirname(ruta_abs)
	regions_txt=ref_dir+ "/regions.txt"
	
	#>10X
	comando_awk1='{ if ($3 >= 10) {sum+=1}} END { printf "%.2f\\n", (sum * 100)/NR}'
	#cmd1= "echo " + sample + " >>"+ mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor10; while read -r chrom start end; do samtools depth -m 0 -a " + secundario_dir + "/" + sample + "_bwa_sort_markedDup.bam -r $chrom | awk '" + comando_awk1 + "'; done < " + regions_txt + " >> " + mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor10"
	#para amplicones:
	cmd1= "echo " + sample + " >>"+ mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor10; while read -r chrom start end; do samtools depth -m 0 -a " + secundario_dir + "/" + sample + "_bwa_sort.bam -r $chrom | awk '" + comando_awk1 + "'; done < " + regions_txt + " >> " + mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor10"
	print(cmd1)
	os.system(cmd1)
	#>20X
	comando_awk2='{ if ($3 >= 20) {sum+=1}} END { printf "%.2f\\n", (sum * 100)/NR}'
	#cmd2= "echo " + sample + " >>"+ mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor20; while read -r chrom start end; do samtools depth -m 0 -a " + secundario_dir + "/" + sample + "_bwa_sort_markedDup.bam -r $chrom | awk '" + comando_awk2 + "'; done < " + regions_txt + " >> " + mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor20"
	cmd2="echo " + sample + " >>"+ mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor20; while read -r chrom start end; do samtools depth -m 0 -a " + secundario_dir + "/" + sample + "_bwa_sort.bam -r $chrom | awk '" + comando_awk2 + "'; done < " + regions_txt + " >> " + mergeresdir + "/horizontal_coverage_by_segment_samtools_depth_mayor20"
	print(cmd2)
	os.system(cmd2)

def segment_depth(dir, sample, mergeresdir, referencia):
	secundario_dir=os.path.join(dir, "secundario")
	...
	#cmd="echo " + sample + " >>" + mergeresdir + "/depth_by_segment_samtools_depth; while read -r chrom start end; do samtools depth -m 0 -a " + secundario_dir + "/" + sample + "_bwa_sort_markedDup.bam -r $chrom | awk '{sum+=$3} END {print sum/NR}'; done < " + regions_txt + " >> " + mergeresdir + "/depth_by_segment_samtools_depth"
	#para amplicones:
	cmd="echo " + sample + " >>" + mergeresdir + "/depth_by_segment_samtools_depth; while read -r chrom start end; do samtools depth -m 0 -a " + secundario_dir + "/" + sample + "_bwa_sort.bam -r $chrom | awk '{sum+=$3} END {print sum/NR}'; done < " + regions_txt + " >> " + mergeresdir + "/depth_by_segment_samtools_depth"
	print(cmd)
	os.system(cmd)
...
def main():
	...
	if args.reference:
		if alignment_tool == "bwa":
			...
			segment_coverage(output,sample, mergeresdir, reference)
