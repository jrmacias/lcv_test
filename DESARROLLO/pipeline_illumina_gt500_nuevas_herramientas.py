...
def quast(dir, sample, scaffold, paired_reads1, paired_reads2, unpaired_reads1, unpaired_reads2, reference="nan", threads=4):  #
	...
	if not reference == "nan":
		cmd="quast.py " + " -o " + assembling_dir + " -t " +threads + " -r " + reference + " --pe1 " + paired_reads1 + " --pe2 " + paired_reads2 + " --single " + unpaired_reads1 + " --single " + unpaired_reads2 + " " + scaffold
		...
		cmd="quast.py " + " -o " + assembling_dir + " -t " +threads + " --pe1 " + paired_reads1 + " --pe2 " + paired_reads2 + " --single " + unpaired_reads1 + " --single " + unpaired_reads2 + " " + scaffold
...
def main():
	...
	#con referencia
	if args.reference:
		quast(output, sample, output+"/assembling/" + sample + ".fasta",  output +"/primario/"+sample+"_trimmed_1P.fastq.gz",  output +"/primario/"+sample+"_trimmed_2P.fastq.gz", output +"/primario/"+sample+"_trimmed_1U.fastq.gz", output+"/primario/"+sample+"_trimmed_2U.fastq.gz", reference, threads)
		...
		#sin referencia
		quast(output, sample, output+"/assembling/" + sample + ".fasta", output +"/primario/"+sample+"_trimmed_1P.fastq.gz",  output +"/primario/"+sample+"_trimmed_2P.fastq.gz", output+"/primario/"+sample+"_trimmed_1U.fastq.gz", output+"/primario/"+sample+"_trimmed_2U.fastq.gz", threads=threads)
