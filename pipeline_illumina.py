...
def createSubdirectories(output, mergeres_dir=None):
	...
	if not mergeres_dir:
		mergeres_dir=os.path.join(parent_dir, "merge_res")
...
def virulencefinder(dir, scaffold, specie="nan"):
	...
	parent_dir=dir
	virulencefinder_dir=os.path.join(parent_dir, "virulencefinder")
	if specie=="nan" or specie != "listeria" or specie != "escherichia_coli" or specie != "staphylococcus_aureus" or specie != "enterococcus_faecalis" or specie != "enterococcus_faecium":
		...
		if specie == "staphylococcus_aureus":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + scaffold + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/virulencefinder_db/ -d s.aureus -l 0.6 -t 0.9 -x"
		if specie == "enterococcus_faecalis":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + scaffold + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/virulencefinder_db/ -d virulence_ent -l 0.6 -t 0.9 -x"
		if specie == "enterococcus_faecium":
			cmd="python /home/admingenomica/software/virulencefinder/virulencefinder.py -i " + scaffold + " -o " + virulencefinder_dir + " -p /home/admingenomica/software/virulencefinder_db/virulencefinder_db/ -d virulence_entfm_entls -l 0.6 -t 0.9 -x"
...
def main():
	...
	#run abricate and merge
	if (args.specie) and (specie == "salmonella"):
		abricate(output, sample, output+"/assembling/" + sample + "_gt500.fasta")
		merge_abricate_results(output, sample, mergeresdir)
	else:
		print("Si -sp es escherichia_coli, campylobacter, klebsiella, enterococcus_faecalis, enterococcus_faecium, staphylococcus_aureus o listeria ejecutar mejor VirulenceFinder")
	...
	#run virulencefinder and merge
	species_virulencefinder=["escherichia_coli", "campylobacter", "klebsiella", "enterococcus_faecalis", "enterococcus_faecium", "staphylococcus_aureus", "listeria"]
	specie=args.specie
	if specie != "salmonella":
		if specie in species_virulencefinder:
		...
		print("VirulenceFinder no se usa para Salmonella. Usar mejor ABRicate")
