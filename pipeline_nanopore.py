...
def createSubdirectories(output_nanopore):
	...
	mlst_dir=os.path.join(parent_dir, "mlst_DTU")
...
def longqc(output_nanopore, minion_read, sample, threads):
	...
	s=" "
	cmd2= "python /home/admingenomica/software/LongQC_1.2.1/longQC.py sampleqc -o " + out_dir + "/longqc_raw_reads" + " -x ont-ligation -s " + sample + " -p " + threads + s + minion_read
	print(cmd2)
	os.system(cmd2)
...
def fastqc(output_nanopore, minion_read, sample, threads):
	...
	print(cmd1)
	os.system(cmd1)
	#CAMBIADO cmd2="fastqc -t " + threads + " -o " + fastqc_dir + "/fastqc/" + s + minion_read
...
def porechop(output_nanopore, sample, minion_read, threads):
	...
	# Replace 'my_environment' with the name of your Conda environment
	environment_name = 'porechop'
	# Command to activate the Conda environment
	activate_command = f'conda run -n {environment_name}'
	# Command to execute your Python script
	python_script = '/labgenomica/scripts/pruebas_conda/ejecutar_porechop.py ' + minion_read + " " + porechop_dir + " " + sample + " " + threads
	# Construct the final command to run
	final_command = f'{activate_command} python {python_script}'
	# Execute the command
	subprocess.call(final_command, shell=True)
...
def fasqc_trimming(output_nanopore, filtered_trimming, sample, threads):
	...
	fastqc_dir=os.path.join(parent_dir, "primario")
	s=" "
	#cambiadocmd1="fastqc -t " + threads + " -o " + fastqc_dir + "/fastqc/" + s + fastqc_dir + "/" + sample + "_trimmed_filtered.fastq.gz"
...
def longqc_trimming(output_nanopore, filtered_trimming, sample, threads):
	...
	s=" "
	#cambiadocmd="python /home/admingenomica/software/LongQC_1.2.1/LongQC.py sampleqc -o " + longqc_dir + "/longQC_trimmed_reads" + s + "-x ont-ligation -s " + sample + s + "-p " + threads + s + filtered_trimming
	cmd="python /home/admingenomica/software/LongQC_1.2.1/longQC.py sampleqc -o " + longqc_dir + "/longQC_trimmed_reads" + s + "-x ont-ligation -s " + sample + s + "-p " + threads + s + filtered_trimming
	print(cmd)
	os.system(cmd)
...
def quast_hybrid(output_illumina, output_nanopore, sample, pair1, pair2, unpair_concatened, filtered_trimming, assembly, threads, reference="nan"):
	print("----------------------")
	print("Start Quast ")
	print("----------------------")
	
	assembling_dir=os.path.join(output_nanopore,"assembling")
	if not reference == "nan":
		...
		activate_command = f'conda run -n {environment_name}'
		# Command to execute your Python script
		python_script = '/labgenomica/scripts/pruebas_conda/ejecutar_quast_hybrid_nanopore.py ' + assembly + " " + pair1 + " " + pair2 + " " + unpair_concatened+" " + assembling_dir + " " + output_illumina + " " + sample + " " + threads + " " + filtered_trimming + " " + reference
		# Construct the final command to run
		final_command = f'{activate_command} python {python_script}'
		# Execute the command
		subprocess.call(final_command, shell=True)
		...
		activate_command = f'conda run -n {environment_name}'
		# Command to execute your Python script
		python_script = '/labgenomica/scripts/pruebas_conda/ejecutar_quast_hybrid_nanopore.py ' + assembly + " " + pair1 + " " + pair2 + " " + unpair_concatened + " " + assembling_dir + " " + output_illumina + " " + sample + " " + threads + " " + filtered_trimming
		# Construct the final command to run
		final_command = f'{activate_command} python {python_script}'
		# Execute the command
		subprocess.call(final_command, shell=True)
...
def quast_only_nanopore(output_nanopore, sample, filtered_trimming, assembly, threads, reference="nan"):
	print("----------------------")
	print("Start Quast")
	print("----------------------")
	...
	if not reference == "nan":
		...
		activate_command = f'conda run -n {environment_name}'
		# Command to execute your Python script
		python_script = '/labgenomica/scripts/pruebas_conda/ejecutar_quast_only_nanopore.py ' + assembly + " " + assembling_dir + " " + sample + " " + threads + " " + filtered_trimming + " " + reference
		# Construct the final command to run
		final_command = f'{activate_command} python {python_script}'
		# Execute the command
		subprocess.call(final_command, shell=True)
		...
		activate_command = f'conda run -n {environment_name}'
		# Command to execute your Python script
		python_script = '/labgenomica/scripts/pruebas_conda/ejecutar_quast_only_nanopore.py ' + assembly + " " + assembling_dir + " " + sample + " " + threads + " " + filtered_trimming
		# Construct the final command to run
		final_command = f'{activate_command} python {python_script}'
		# Execute the command
		subprocess.call(final_command, shell=True)
...
def plasmidfinder(output_nanopore, assembly):
	...
	cmd="/home/admingenomica/software/plasmidfinder/plasmidfinder.py -i " + assembly + " -o " + plasmidfinder_dir + " -x -p /home/admingenomica/software/plasmidfinder_db/plasmidfinder_db/" + " -t 0.80"
	print(cmd)
	os.system(cmd)
...
def seqsero2(output_nanopore, scaffold, sample): #Salmonella serotyping
...
def sistr(output_nanopore,sample,merge, scaffold): #Salmonella serotyping
	print("Start sistr")
	print("----------")
...
def serotyfinder(output_nanopore, scaffold): #Ecoli serotyping
...
def resfinder(output_nanopore, assembly, specie="nan"):
	...
	resfinder_dir=os.path.join(parent_dir, "resfinder")
	if (specie=="nan"):
		cmd="python /home/admingenomica/software/resfinder/run_resfinder.py -ifa " + assembly + " -o " + resfinder_dir + " -acq -db_res /home/admingenomica/software/resfinder_db/resfinder_db/ -c -db_point /home/admingenomica/software/resfinder_db/pointfinder_db/" 
	else:
		if specie == "escherichia_coli":
			specie = "ecoli"
		cmd="python /home/admingenomica/software/resfinder/run_resfinder.py -ifa " + assembly + " -o " + resfinder_dir + " -s " + specie + " -acq -db_res /home/admingenomica/software/resfinder_db/resfinder_db/ -c -db_point /home/admingenomica/software/resfinder_db/pointfinder_db/"
	print(cmd)
	os.system(cmd)
...
def main():
	...
	if assembly == "hybrid":
		...
		#Calidad ensamblado
		quast_hybrid(output_illumina, output_nanopore, sample, output_illumina+"/primario/"+sample+"_trimmed_1P.fastq.gz", output_illumina+"/primario/"+sample+"_trimmed_2P.fastq.gz", output_illumina + "/primario/" + sample + "_trimmed_allU.fastq.gz", output_nanopore+"/primario/"+ sample+"_trimmed_filtered.fastq.gz", output_nanopore+"/assembling/assembly.fasta", threads , reference="nan")
		...
		#calidad ensamblado
		quast_only_nanopore(output_nanopore, sample, output_nanopore+"/primario/"+ sample+"_trimmed_filtered.fastq.gz", output_nanopore+"/assembling/assembly.fasta", threads , reference="nan")
	...
	if (args.specie) and (args.specie != "nan"):
		#run mlst_DTU
		mlst_DTU(output_nanopore, output_nanopore+"/assembling/assembly.fasta", specie)	
